      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ background: 'var(--sur)', border: '1px solid var(--bd)', borderRadius: 16, padding: '28px 24px' }}>
          <div style={{ marginBottom: 24 }}>
            <div style={{ fontSize: 17, fontWeight: 700, color: 'var(--f1)', letterSpacing: '-0.02em' }}>계정 만들기</div>
          ...
          {state?.success ? (
            ...
            <form action={action} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--f3)' }}>이름</label>
                <input name="full_name" type="text" autoFocus autoComplete="name" placeholder="홍길동" style={inputStyle} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--f3)' }}>이메일</label>
                <input name="email" type="email" required autoComplete="email" placeholder="name@bcave.co.kr" style={inputStyle} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--f3)' }}>비밀번호</label>
                <input name="password" type="password" required autoComplete="new-password" placeholder="8자 이상" style={inputStyle} />
              </div>
