...
// ── 통계 탭 ──────────────────────────────────────────────────────
function PromoStats() {
  type Window = '30D' | '90D' | '1Y';
  const [win,     setWin]     = React.useState<Window>('90D');
  const [loading, setLoading] = React.useState(true);
  ...
  React.useEffect(() => {
    ...
    Promise.all([
      sb.from('promotions').select('promotion_type, snapshot_date').gte('snapshot_date', fromStr).limit(1000),
      sb.from('promotion_items').select('promotion_id, musinsa_brand_name, discount_rate, snapshot_date').gte('snapshot_date', fromStr).limit(5000),
    ]).then(([promoRes, itemRes]) => {
      const promoRows = (promoRes.data  ?? []) as any[];
      const itemRows  = (itemRes.data   ?? []) as any[];

      // 타입별 건수
      const promosByType: Record<string, number> = {};
      for (const p of promoRows) promosByType[p.promotion_type] = (promosByType[p.promotion_type] ?? 0) + 1;

      // 할인폭 분포
      const discountBins = [0,0,0,0,0,0,0,0,0];
      const binEdges     = [5,10,15,20,25,30,40,50,Infinity];
      let totalDr = 0, cntDr = 0;
      for (const item of itemRows) {
        const dr = item.discount_rate ? parseFloat(item.discount_rate) : 0;
        if (dr > 0) { totalDr += dr; cntDr++; }
        for (let i = 0; i < binEdges.length; i++) {
          if (dr < binEdges[i]) { discountBins[i]++; break; }
        }
      }
      ...
      const brandCounts: [string, number][] = Object.entries(brandMap).sort((a, b) => b[1] - a[1]).slice(0, 7);

      // 일별 프로모션 수 (최근 12일)
      const dailyCounts = new Array(12).fill(0);
      const todayStr    = kstDateStr();
      for (const p of promoRows) {
        const diff = Math.floor((new Date(todayStr).getTime() - new Date(p.snapshot_date).getTime()) / 86400000);
        const idx  = 11 - diff;
        if (idx >= 0 && idx < 12) dailyCounts[idx]++;
      }
  ...
  if (loading) return (
    <div className="panel" style={{ padding: 40, color: 'var(--f4)', textAlign: 'center', fontSize: 13 }}>
      통계 로딩중…
    </div>
  );
  if (!stats) return null;

  const typeTotal = Object.values(stats.promosByType).reduce((a, b) => a + b, 0);
  const typeDistPct = Object.entries(TYPE_LABEL)
    .map(([k, label]) => {
      const count = stats.promosByType[k] ?? 0;
      return { key: k, label, pct: typeTotal > 0 ? Math.round(count / typeTotal * 100) : 0, count };
    })
    .sort((a, b) => b.count - a.count);
  const topType      = typeDistPct[0] ?? { label: '—', pct: 0 };
  const maxBrandCnt  = stats.brandCounts[0]?.[1] ?? 1;
  const maxDailyCnt  = Math.max(...stats.dailyCounts, 1);
    ...
    <>
      ...
      <div className="grid grid-2 gap-12">
        <section className="panel">
          ...
          <Bars h={120} values={stats.dailyCounts.map(v => v / maxDailyCnt * 100)} />
        </section>
        <section className="panel">
          <div className="sec-head"><h3>할인폭 분포 <span className="sub">상품 기준</span></h3></div>
          <Bars h={120}
            values={stats.discountBins}
            accentIdx={stats.discountBins.indexOf(Math.max(...stats.discountBins))} />
      ...
      <div className="grid grid-3 gap-12">
        <section className="panel">
          <div className="sec-head">
            <h3>타입 분포 <span className="sub">총 {stats.totalPromos}건</span></h3>
          </div>
          <div className="row-flex center gap-12">
            <Donut size={84} percent={topType.pct} label={`${topType.pct}%`} sub={topType.label} />
            <div className="flex-1">
              {typeDistPct.map(({ key, label, pct, count }) => (
                <div key={key} className="row-flex between" style={{ padding: '3px 0' }}>
                  <span style={{ fontSize: 11, color: 'var(--f2)', display: 'flex', alignItems: 'center', gap: 4 }}>
                    <span className="pip" style={{ display: 'inline-block' }} />
                    {label}
                  </span>
                  <span className="mono dim" style={{ fontSize: 11 }}>{count} · {pct}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="panel">
          <div className="sec-head"><h3>브랜드 TOP 7 <span className="sub">상품 수 기준</span></h3></div>
          {stats.brandCounts.length === 0 ? (
            <span className="dim" style={{ fontSize: 12 }}>데이터 없음</span>
          ) : stats.brandCounts.map(([name, count], i) => (
            <div key={i} className="row-flex center gap-8" style={{ padding: '4px 0' }}>
              <span style={{ flex: 1, fontSize: 12, color: 'var(--f2)' }}>{name}</span>
              <HBar value={count} max={maxBrandCnt} accent={i === 0} w={90} />
              <span className="mono dim" style={{ fontSize: 11, width: 28, textAlign: 'right' }}>{count}</span>
            </div>
          ))}
        </section>
        ...
        <section className="panel surface">
          ...
          <p style={{ margin: 0, fontSize: 12, lineHeight: '19px', color: 'var(--f2)' }}>
            {stats.totalPromos === 0 ? (
              <span style={{ color: 'var(--f4)' }}>수집된 데이터가 없습니다.</span>
            ) : (
              <>
                <span className="hs" style={{ fontWeight: 500 }}>{topType.label}</span>이 전체 중{' '}
                {topType.pct}%로 가장 많음.{' '}
                평균 할인율 {stats.avgDiscount != null ? `${stats.avgDiscount.toFixed(1)}%` : '—'} ·{' '}
                수집 상품 {stats.totalItems.toLocaleString()}건.
                {stats.brandCounts[0] ? ` ${stats.brandCounts[0][0]} 브랜드가 상품 수 기준 1위.` : ''}
              </>
            )}
          </p>
        </section>
