...
## 규칙
- insights는 정확히 10개
- DART 공시(dart_disclosures)가 있으면 insights[0]에 최우선 배치 — 절대 누락 금지, link는 /company?id=<companies.id>
- news_picks는 입력 external_news 중 relevance ≥ 4 만, 최대 5개
- 자사 영향이 큰 순으로 정렬
- 데이터에 없는 사실 절대 만들지 마라
- 링크: 회사 정보는 /company?id=<companies.id>, 이상탐지는 /anomaly?id=<anomaly_id>
...
- 링크: 상품은 /product?id=<product_id>, 브랜드는 /brand?slug=<slug>, 랭킹은 /ranking
...
async def run(
    ...
    if failed_idx:
        ...
        logger.warning("briefing_retry_scheduled", failed=failed_auds, wait_min=30)
